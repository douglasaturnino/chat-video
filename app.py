    ...
    if uploaded_file is not None:
        if not is_video_file(uploaded_file):
            ...
            if file_size > 100:  # Gemini has file size limits
                st.warning(
                    "Large files may take longer to process or may fail. Consider compressing your video."
                )
            ...
            if (
                ...
                if st.session_state.video_processor is None:
                    ...
                    with st.spinner(
                        ...
                        try:
                            video_file = (
                                st.session_state.video_processor.upload_video(
                                    tmp_file_path, uploaded_file.name
                                )
                            )

                            if video_file:
                                processed_file = st.session_state.video_processor.wait_for_file_processing(
                                    video_file
                                )
...
<div style='text-align: center; color: #666;'>
    <p><a href='https://ai.google.dev/gemini-api/docs/video-understanding' target='_blank'>Learn more about Gemini Video API</a></p>
</div>
""",
